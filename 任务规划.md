...
│
├─ docs/zh-cn/算法组考核/               ★ 本文件夹（作业文档，独立仓库 RMCS_Navigation）
│   ├─ 导航提交作业.md                  ⭐ 提交报告（验收用）
│   ├─ Foxglove观测指南.md              ★ 怎么看动态过程
│   ├─ 实验记录.md                      原始数据
│   ├─ 任务规划.md                      ← 本文件
│   ├─ config/                          配置快照
│   └─ tools/virtual_chassis.py         虚拟底盘（让静态环境动起来）
│
├─ rmcs_ws/                            ROS2 工作空间
...
**四个必踩的坑**（详见 [导航提交作业.md](导航提交作业.md) §5）：
...
|---|---|
| **目标** | 确认基线、搞清改哪里 |
| **操作** | ① 读 [导航提交作业.md](导航提交作业.md) 全文<br>② **问组长**：基线确认 + 是否允许用上游 `main`（见下） |
...
|---|---|
| **目标** | 无机器人跑起导航栈，建立信心 |
| **操作** | ① 按 §2.3 构建<br>② 启动 `static.launch.yaml`<br>③ 另开终端检查<br>④ **在 Foxglove 里观察** |
| **验收** | 4 节点全 `active`；能查到规划器类名；能在 Foxglove 看到 `/plan` |
...
> ⭐ **要让画面动起来**（机器人真的移动、代价地图滚动），需要额外一步：
> 杀掉静态 TF 并用 `tools/virtual_chassis.py` 发布动态 TF。
> 完整步骤见 **[Foxglove观测指南.md](Foxglove观测指南.md)**。
...
| 验收问题 | 怎么答 |
|---|---|
| **为什么挑这个规划器？和现有差异？** | 一句话：*选 Smac2D 因为它内置路径平滑 + 多分辨率降采样 + 显式障碍代价，正好治 NavFn 的折线转折降速和单分辨率全图慢。* 展开讲三点（见 [导航提交作业.md](导航提交作业.md) §2.4） |
...
| 仓库 | 内容 | 何时用 |
|---|---|---|
| **`RMCS_Navigation`**（本文件夹） | 提交报告、过程规划、配置快照、演示材料 | ✅ **过程中都提交到这里** |
| `rmcs-navigation`（fork） | `motion.yaml` 等代码改动 | 规划器改动后 |
| `RMCS_Test`（作业主仓） | 最终版 | ⏳ **最后才提交** |

### 8.2 过程中的提交

```bash
# 本文件夹是一个独立 git 仓库，remote = RMCS_Navigation
cd /workspaces/RMCS/docs/zh-cn/算法组考核

git add -A
git commit -m "报告: 补充 XX"
git push          # → RMCS_Navigation
```
...
git add config/motion.yaml
git commit -m "feat(nav): 全局规划器由 NavFn 切换为 SmacPlanner2D"
git push          # → origin = gebilaowuxiaoyu123/rmcs-navigation
```

### 8.3 最终版提交回 `RMCS_Test`

工作完成后，把本文件夹的**最终内容**并入作业主仓：
...
# ③ 回到作业主仓，去掉 .gitignore 里的忽略规则
cd /workspaces/RMCS
# 编辑 .gitignore，删除这行：/docs/zh-cn/算法组考核/
...
### 8.4 为什么过程中不入 `RMCS_Test`

- 考核内容与已完成的周任务分开，避免作业错乱；
- 过程中改动频繁，不必污染主仓历史；
- 最终版一次性并入，主仓历史干净。

### 8.5 检查清单

- [ ] 本文件夹已 `git push` 到 `RMCS_Navigation`
- [ ] `motion.yaml` 改动已 `git push` 到 fork
- [ ] 报告 §0.2 完成度表已更新为实际状态
- [ ] 报告 §4.2 已填入实测对比数据
- [ ] `assets/` 已放入基线 / 对比录屏
- [ ] `config/motion.yaml.smac` 快照已补
- [ ] 最终版按 §8.3 并入 `RMCS_Test`
