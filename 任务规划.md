...
```bash
# 构建（要点：bash -lc 包一层、PATH 加 /opt/cmake、用 colcon）
...
**为什么每个命令都套 `bash -lc`？** 因为你在 zsh 里，
`source /opt/ros/jazzy/setup.bash` 会**静默失效**——那个脚本用 `BASH_SOURCE[0]` 找自己，
zsh 里这个变量是空的，于是它跑到当前目录找 `setup.sh`，报
`no such file: .../rmcs_ws/setup.sh`。套一层 bash 就没这问题。

| 坑 | 现象 | 怎么办 |
|---|---|---|
| zsh 下 source ROS 失效 | `no such file: .../setup.sh` | 用 `bash -lc '...'` 包一层 |
| CMake 太旧 | `CMake 3.30 or higher is required`（系统是 3.28） | `export PATH=/opt/cmake/bin:$PATH`（有 4.2.3） |
| `build-rmcs` 报错 | `install directory was created with the layout 'isolated'` | 别用 `build-rmcs`（它默认 `--merge-install`），直接用 `colcon build` |
...
| 操作 | 改 `config/motion.yaml` → 重新构建 → 重启 → 验证 |
...
# —— 频率（"无卡顿"的客观证据）——
bash -lc 'cd /workspaces/RMCS/rmcs_ws && source install/setup.bash && ros2 topic hz /plan'
```
